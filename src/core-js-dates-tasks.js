...
function getNextFriday(date) {
  const resulDate = new Date(date);
  const dayOfWeek = resulDate.getUTCDay();
  const dayUnilFriday = (5 - dayOfWeek + 7) % 7 || 7;
  resulDate.setUTCDate(resulDate.getUTCDate() + dayUnilFriday);
  return resulDate;
}
...
function isDateInPeriod(date, period) {
  const checkeDate = new Date(date);
  const startDate = new Date(period.start);
  const endDate = new Date(period.end);

  return checkeDate >= startDate && checkeDate <= endDate;
}
...
function formatDate(date) {
  ...
  const formattedDate = dates.toLocaleString('en-US', options);
...
function getWeekNumberByDate(date) {
  ...
  const dayToMonday = dayOfWeek <= 4 ? 1 - dayOfWeek : 8 - dayOfWeek;

  firstDayOfYear.setDate(firstDayOfYear.getDate() + dayToMonday);

  const diff = currentDate - firstDayOfYear;
